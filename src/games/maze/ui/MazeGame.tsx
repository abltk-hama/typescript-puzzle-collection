...
export function MazeGame({
  ...
  useEffect(() => {
    ...
    if (complete || (state.history.length === 1 && !state.hintCount))
  ...
  function apply(next: MazeState) {
    if (next === state) return;
    setState(next);
    setMessage(
      isComplete(puzzle, next)
        ? `ゴールに到着しました！（${next.history.length - 1}手、ヒント使用 ${next.hintCount}回）`
        : "一マス進みました。",
    );
  }
  function direction(row: number, column: number) {
    apply(moveDirection(puzzle, state, row, column));
  }
  function keyDown(event: React.KeyboardEvent) {
    ...
    if (mapping[event.key]) {
      ...
      apply(moveDirection(puzzle, state, ...mapping[event.key]));
    }
  }
  if (phase === "loading")
    return (
  ...
  const visits = new Set(state.history.map(keyOf));
  return (
    <main className="shell">
      <section className="panel">
        ...
        <div className="stats">
          <span>手数: {state.history.length - 1}</span>
        ...
        <div className="board-wrap">
          <div
            ...
            style={{
              ...
              ["--maze-size" as string]: puzzle.width,
            }}
          >
            {puzzle.passages.map((mask, index) => {
              const position = {
                  row: Math.floor(index / puzzle.width),
                  column: index % puzzle.width,
                },
                isCurrent = samePosition(position, current),
                isStart = samePosition(position, puzzle.start),
                isGoal = samePosition(position, puzzle.goal),
                isHint =
                  state.hintPosition &&
                  samePosition(position, state.hintPosition);
              return (
                <button
                  key={index}
                  role="gridcell"
                  aria-label={`${position.row + 1}行${position.column + 1}列${isCurrent ? " 現在位置" : ""}${isStart ? " 入口" : ""}${isGoal ? " ゴール" : ""}`}
                  className={`maze-cell ${visits.has(keyOf(position)) ? "visited" : ""} ${isStart ? "start" : ""} ${isGoal ? "goal" : ""} ${isHint ? "hinted" : ""}`}
                  style={{
                  ...
                  onClick={() => apply(moveTo(puzzle, state, position))}
                  disabled={complete}
                >
        ...
        <div className="actions">
          <button
            ...
            onClick={() => {
              ...
              setState(next);
              if (next === state) {
                setMessage("表示中のヒントを維持します。");
                return;
              }
              const hint = next.hintPosition!,
                dr = hint.row - current.row,
                dc = hint.column - current.column;
              setMessage(
                `${directionNames[`${dr},${dc}`]}のマスへ進んでください。`,
              );
            }}
          >
            次に進むマスを表示
        ...
        <GameFooter
          ...
          left={
            <>
              <button
                className="button secondary"
                disabled={state.history.length <= 1 || complete}
                onClick={() => {
                  setState(undo(state));
                  setMessage("一手戻しました。");
                }}
