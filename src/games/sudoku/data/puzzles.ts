...
export async function loadBundledPuzzles(){const response=await fetch('/puzzles/sudoku/puzzles.json');if(!response.ok)throw new Error('数独問題を読み込めませんでした。');return parsePuzzles(await response.json())}
